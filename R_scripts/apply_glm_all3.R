library(ade4)
all <- read.csv("/Users/jacobluber/Desktop/athlete_metadata/lactate/l_genomics.csv", header = TRUE, sep=",")
...
for (i in 2:57){
  results[i] <- wilcox.test(x=after[[important_cols[i]]],y=before[[important_cols[i]]],alternative="greater")$p.value
}
joined_stats_tests <- data_frame(important_cols[2:57],results[2:57])
important_results <- filter(joined_stats_tests,results[2:57] < 0.025)
write.csv(important_results,"lactate_greater.csv")
